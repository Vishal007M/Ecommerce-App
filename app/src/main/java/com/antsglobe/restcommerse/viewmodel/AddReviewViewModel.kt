...
class AddReviewViewModel(private val apiService: ApiService) : ViewModel() {
    ...
    fun AddReviewResponse(email: String, productId: String, review: String, rating: String, img: String, img_url: String) {
        if (!NetworkUtils.isNetworkConnected()) {
            NetworkUtils.showToast()
            return
        }
        apiService.addCustomerReviews(email, productId, review, rating,img, img_url).enqueue(object :
